...
import { MessageSquarePlus, Trash2, Search, X, MessageSquare } from 'lucide-react';
...
export function ModelSidebar({
  ...
  const { data: conversations = [], mutate } = useSWR<ConversationItem[]>('/api/conversations?mode=model', fetcher);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(`/api/conversations/search?mode=model&q=${encodeURIComponent(searchQuery)}`);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="font-semibold text-sm">大模型对话</span>
        </div>
        <Button
          onClick={onNewChat}
