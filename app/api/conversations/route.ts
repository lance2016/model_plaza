...
export async function GET(req: Request) {
  try {
    ...
    const conversations = getAllConversations(mode || undefined);
...
export async function POST(req: Request) {
  try {
    ...
    const { model_id, title, messages, agent_id } = body;
    if (!model_id) {
      return NextResponse.json({ error: 'Missing required field: model_id' }, { status: 400 });
    }
    const id = uuidv4();
    createConversation({ id, model_id, title, messages, agent_id });
